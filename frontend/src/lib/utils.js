 ...
 */
export function handleDuration(time) {
    time = parseInt(time);
    if (isNaN(time)) {
        return "未知时间";
    }
 ...
 */
export function formatMonthDayTime(dateTime) {
    // 将传入的日期时间转换为 Date 对象
    const inputDate = new Date(dateTime);
    // 如果转换后的日期无效，返回 '未知时间'
    if (isNaN(inputDate.getTime())) {
 ...
 */
export function formatStandardDateTime(dateTime) {
    const inputDate = new Date(dateTime);
    if (isNaN(inputDate.getTime())) {
        return "未知时间";
    }
...
// 导出一个异步函数，用于将图片URL转换为Blob URL
export async function convertToBlobUrl(imageUrl) {
    try {
        // 使用fetch函数获取图片URL的响应
