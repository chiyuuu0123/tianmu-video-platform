<template>
  <div class="index">
    ...
    <!-- 固钉频道栏 -->
    <div
      class="header-channel-fixed"
      :style="isFixChannel ? '' : 'display: none;'"
    >
      <div
          ...
          <div class="header-channel-fixed-right-left">
            <div class="left-top">
              <a
                ...
                :style="
                  $route.params.categoryId == item.categoryId
...
import { fetchVideoCateList } from "@/api/video";
...
export default {
  ...
  computed: {
    ...
    categoryName() {
      let index = this.categoryList.findIndex(
        (item) => item.categoryId === parseInt(this.$route.params.categoryId)
      );
      if (index !== -1) {
        return this.categoryList[index].categoryName;
      }
      return "";
    },
  },
  methods: {
    async getVideoList() {
      try {
        const res = await fetchVideoCateList(this.$route.params.categoryId);
        if (res.code === 200) {
          this.cateVideoList = res.data;
...
.category-page__head .category-name {
  font-size: 28px;
  line-height: 40px;
  font-weight: 500;
  color: var(--text1);
