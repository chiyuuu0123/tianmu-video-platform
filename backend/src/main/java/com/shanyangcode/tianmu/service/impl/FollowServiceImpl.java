...
public class FollowServiceImpl extends ServiceImpl<FollowMapper, Follow>
    ...
    public boolean follow(FollowRequest followRequest) {
        ...
        ThrowUtils.throwIf(followRequest.getUserId().equals(followRequest.getCreatorId()),
        ...
        boolean alreadyFollowed = this.lambdaQuery()
                .eq(Follow::getUserId, followRequest.getUserId())
                .eq(Follow::getCreatorId, followRequest.getCreatorId())
                .exists();
        ThrowUtils.throwIf(alreadyFollowed, ErrorCode.OPERATION_ERROR, "已经关注该用户");
    ...
    public boolean chanelFollow(FollowRequest followRequest) {
        ...
        ThrowUtils.throwIf(!this.exists(queryFollowWrapper), ErrorCode.OPERATION_ERROR, "尚未关注该用户");

        boolean removed = this.remove(queryFollowWrapper);
        ThrowUtils.throwIf(!removed, ErrorCode.SYSTEM_ERROR, "取消关注失败");

        // 更新粉丝统计
        boolean updatedFollowers = userStatsService.lambdaUpdate().setSql("followers = GREATEST(followers - 1, 0)").eq(UserStats::getUserId, followRequest.getCreatorId()).update();
        ThrowUtils.throwIf(!updatedFollowers, ErrorCode.SYSTEM_ERROR, "更新博主粉丝统计失败");

        // 更新关注统计
        boolean updatedFollowing = userStatsService.lambdaUpdate().setSql("following = GREATEST(following - 1, 0)").eq(UserStats::getUserId, followRequest.getUserId()).update();
        ThrowUtils.throwIf(!updatedFollowing, ErrorCode.SYSTEM_ERROR, "更新用户关注统计失败");
